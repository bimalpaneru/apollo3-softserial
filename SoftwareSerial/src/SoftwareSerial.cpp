/*
  ...
  Limitations (similar to Arduino core SoftwareSerial):
    ...
    * TX gets priority. So if Artemis is receiving a string of characters
    and you do a Serial.print() the print will begin immediately and any
  additional RX characters will be lost.
    ...
    * Enabling multiple ports causes 19200 RX to fail (because there is
  additional instance switching overhead)
...
SoftwareSerial::~SoftwareSerial() {
  end();  // End detaches the interrupt which removes the reference to this
          // object that is stored in the GPIO core
}

// start 8 bits, no parity, 1 stopbit
void SoftwareSerial::begin(uint32_t baudRate) { begin(baudRate, SERIAL_8N1); }
...
void SoftwareSerial::listen() {
  ...
  if (ap3_active_softwareserial_handle != NULL)
    ap3_active_softwareserial_handle
        ->stopListening();  // Gracefully shut down previous instance
...
void SoftwareSerial::begin(uint32_t baudRate, uint16_t SSconfig) {
  ...
  am_hal_stimer_config(AM_HAL_STIMER_HFRC_3MHZ |
                       AM_HAL_STIMER_CFG_COMPARE_H_ENABLE);
  ...
  rxSysTicksPerBit = (TIMER_FREQ / baudRate) *
                     0.98;  // Shorten the number of sysTicks a small amount
                            // because we are doing a mod operation
  txSysTicksPerBit = (TIMER_FREQ / baudRate) -
                     comp;  // Shorten the txSysTicksPerBit by the number of
                            // ticks needed to run the txHandler ISR

  rxSysTicksPerByte =
      (TIMER_FREQ / baudRate) * (_dataBits + _parityBits + _stopBits);
...
int SoftwareSerial::available() {
  return (rxBufferHead + AP3_SS_BUFFER_SIZE - rxBufferTail) %
         AP3_SS_BUFFER_SIZE;
}
...
void SoftwareSerial::beginTX() {
  ...
  AM_REGVAL(AM_REG_STIMER_COMPARE(0, 7)) =
      txSysTicksPerBit;  // Direct reg write to decrease execution time
 ...
 * the rxBIT routine.(bypassing Mbed) In the Apollo3 driver, routine
 * am_gpio_isr(), takes 3.5us to get data, 10us to detect which pin generated
 * the interrupt and another 2.7us to read the information which interrupt
 * routine to call next with the right data. The last uS's are needed to make
 * the call.
 *
 * This routine itself takes 3.3us if the level change happend after
 * 1 bit time, 4.6us after 2 bit times, 6.9us after 4 bit times, 7.8us after 5
 * bits. so about 1.2us for an extra bit.
 *
 ...
 *
 * Let's use questionmark (0x3f). The LSB is sent first so 6 x 1 bit/high
 * followed by 2 x 1 bit/low. At 38400 baud 6 x 26us = 156us then the level
 * changes from high to low for 2 x 26us = 52us. This routine is called 18us
 * after the level change and takes 8.9us. Thus after 27us it returns, BUT as we
 * receive 2 bits we have 52us to complete. Well ontime and we receive the bits
 * and the full character correctly.
 *
 * The character where this becomes of a problem is DEL (0x7F), because then
 * have 7 bits/high and only 1 low bit...
 *
 * Handle a readable data can be done on 38400. However when use digital data
 * e.g. from a sensor, this can be in the range of 0x0 - 0xff stick to 19200.
 *
...
void SoftwareSerial::rxBit(void) {
  ...
  if (lastBitTime == 0) {
    ...
    AM_REGVAL(AM_REG_STIMER_COMPARE(0, 7)) =
        rxSysTicksPerByte;  // Direct reg write to decrease execution time
    ...
    while (numberOfBits--)  // Add bits of the current bitType (either 1 or 0)
                            // to our byte
    {
#ifdef TIME_DEBUG_PIN
      // am_hal_gpio_output_set(TIME_DEBUG_PIN);       // show the number of
      // bits detected on the time debug pin
#endif
      incomingByte >>= 1;
    ...
    lastBitTime = bitTime;  // Remember this bit time as the starting time for
                            // the next PCI
  }
...
//  Serial.printf("incoming: 0x%02X\n", incomingByte);
  ...
  // Edge case where we need to do an additional byte shift because we had data
  // bits followed by a parity bit of same value
  if (_parity) {
    bitCounter = bitCounter - _parityBits;  // Remove parity bit from count
    if (bitType == true) bitCounter++;
  }
...
//        Serial.println("Add bit");
  ...
  if (_invertLogic) incomingByte = ~incomingByte;
  ...
  am_hal_gpio_interrupt_clear(AM_HAL_GPIO_BIT(
      digitalPinToInterrupt(_rxPin)));  // Clear any residual PCIs
...
void SoftwareSerial::txHandler() {
  ...
  {
#ifdef TIME_DEBUG_PIN
    // am_hal_gpio_output_set(TIME_DEBUG_PIN);       // show the number of bits
    // detected on the time debug pin
#endif
...
#ifdef TIME_DEBUG_PIN
    //   am_hal_gpio_output_clear(TIME_DEBUG_PIN);       // show the number of
    //   bits detected on the time debug pin
#endif
  } else if (bitCounter == _dataBits)  // Send parity bit or stop bit(s)
  {
    if (_parity) {
      ...
      AM_REGVAL(AM_REG_STIMER_COMPARE(0, 7)) =
          txSysTicksPerStopBit;  // Direct reg write to decrease execution time
      digitalWrite(_txPin, _invertLogic ? LOW : HIGH);
    }
    bitCounter++;
  }
